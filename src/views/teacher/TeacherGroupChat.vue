...
export default {
    ...
    data(){
        return {
            ...
            time:"",
            textarea: {detail:"",sendTime:"",senderID:this.$store.getters.user.id,teamID:this.$route.params.teamID},
            myId:this.$store.getters.user.id,
            myName:this.$store.getters.user.name,
            myTeam:this.$route.params.teamID,
            url:this.$store.getters.user.avatarURL,
            infos:[],
            users:[
                {name:"",avaterURL: "",teamId:"",id:''}
            ],
            send: {detail:"",sendTime:'',senderID:this.$store.getters.user.id,teamID:this.$route.params.teamID}

        }
    },
    
    mounted() {
        console.log('当前teamID:'+this.$route.params.teamID+'，类型为:'+typeof this.$route.params.teamID)
        getAllUserList().then((res)=>{
            if (res.code===200 ){
                // console.log("getAllUserList success")
        ...
        getMessageList(this.$route.params.teamID).then((res)=>{
            console.log("getMessageList success")
            console.log("id="+this.$route.params.teamID)
            if (res.code===200){
                let resultbody = res.data
